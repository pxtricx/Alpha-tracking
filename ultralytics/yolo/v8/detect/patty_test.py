...
def save_object_image(crop_img, obj_id, frame, save_dir):
    obj_folder = save_dir / f"obj_id_{obj_id}"
        ...
        if len(outputs) > 0:
            ...
            for i, identity in enumerate(identities):
                ...
                if obj_id in object_images:
                    saved_img = object_images[obj_id]
                    if not is_similar_image(saved_img, crop_img):
                        object_images[obj_id] = crop_img
                        save_object_image(crop_img, obj_id, frame, self.save_dir)
                else:
                    object_images[obj_id] = crop_img
                    save_object_image(crop_img, obj_id, frame, self.save_dir)
